#!/bin/bash
if [ "$(whoami)" != "root" ]; then sudo su -s "$0" fi
...
if [ -d "$DIR0" ]; then DEV=$(cat /sys/class/video4linux/video0/name | cut -c 1-4)
inogeni_type
...
if [ -d "$DIR1" ]; then DEV=$(cat /sys/class/video4linux/video1/name | cut -c 1-4)
inogeni_type
if [ $DEVICE = 'plug-n-play' ] && [ $VID_DEV = 'video_pnp0' ]; then VID_DEV=video_pnp1
elif [ $DEVICE = 'plug-n-play' ]; then VID_DEV=video_pnp0
else VID_DEV=video_airplay
#sed -i 's/.*'$VID_DEV'.*/SUBSYSTEM=="video4linux", ATTR{name}=="'$DEV'-INOGENI DVIUSB", SYMLINK += "'$VID_DEV'"/' '$VID_DIR'
fi

if [ -d "$DIR2" ]; then DEV=$(cat /sys/class/video4linux/video2/name | cut -c 1-4)
inogeni_type
